...
def sysCall_init():
    sim = require('sim')
    self.robot1 = sim.getObjectHandle('/PioneerP3DX[1]')
    self.motorLeft=sim.getObject("/PioneerP3DX[1]/leftMotor")
    self.motorRight=sim.getObject("/PioneerP3DX[1]/rightMotor")
    self.Kp_rho = 0.75
    self.Kp_alpha = 1.8
    self.Kp_beta = 0.6
    self.length = 0.381
    ...
    self.vRight = 0
    for i in range(16):
        self.usensors.append(sim.getObjectHandle(f'/PioneerP3DX[1]/ultrasonicSensor[{i}]'))
...
def normal_behavior():
    ...
    target = getTargetPosition('/Cylinder[1]')
    robot = getTargetPosition('/PioneerP3DX[1]')
    x_diff = target[0] - robot[0]
    y_diff = target[1] - robot[1]
    orientation = getTargetOrientation('/PioneerP3DX[1]')
